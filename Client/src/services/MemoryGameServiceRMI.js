...
export async function startMemorySession(category) {
  const res = await fetch(`${API_BASE}/game-sessions/start-memory`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
    body: JSON.stringify({ gameType: "memory", category }),
  });
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`startMemorySession failed: ${res.status} - ${text}`);
...
export async function joinMemoryGame(sessionId) {
  const res = await fetch(
    `${API_BASE}/game-sessions/join-memory?sessionId=${sessionId}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
...
export async function getOpponentB(sessionId) {
  const response = await fetch(
    `${API_BASE}/game-sessions/get-opponent-b?sessionId=${sessionId}`,
    { method: "GET", credentials: "include" }
  );
  if (!response.ok) throw new Error("Failed to get memory game opponent");
  return await response.json();
}

export async function getOpponentA(sessionId) {
  const response = await fetch(
    `${API_BASE}/game-sessions/get-opponent-a?sessionId=${sessionId}`,
    { method: "GET", credentials: "include" }
  );
  if (!response.ok) throw new Error("Failed to get memory game opponent");
  return await response.json();
...
export function checkAnswer({
  ...
  return fetch(`${API_BASE}/game-sessions/check-answer`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
    body: JSON.stringify({
...
export async function getMemoryGameStatus(sessionId) {
  const response = await fetch(
    `${API_BASE}/game-sessions/memory-status?sessionId=${sessionId}`,
    { method: "GET", credentials: "include" }
  );
  if (!response.ok) throw new Error("Failed to get memory game status");
  return await response.json();
}

export function flipQuestion({ sessionId, selectedQuestionCard }) {
  return fetch(
    `${API_BASE}/game-sessions/flip-question?sessionId=${sessionId}&questionIndex=${selectedQuestionCard}`,
...
export function flipAnswer({ sessionId, selectedAnswerCard }) {
  return fetch(
    `${API_BASE}/game-sessions/flip-answer?sessionId=${sessionId}&answerIndex=${selectedAnswerCard}`,
    { method: "POST", credentials: "include" }
  );
}
...
export async function getMemoryGameState(sessionId) {
  const response = await fetch(
    `${API_BASE}/game-sessions/memory-state?sessionId=${sessionId}`,
    { method: "GET", credentials: "include" }
  );
  if (!response.ok) throw new Error("Failed to get memory game state");
  return await response.json();
}

export function getGameResult(sessionId) {
  return fetch(
    `${API_BASE}/game-sessions/result-memory?sessionId=${sessionId}`,
    { method: "GET", credentials: "include" }
  ).then((res) => res.json());
}
...
export function deleteMemoryEntries(sessionId) {
  return fetch(
    `${API_BASE}/game-sessions/delete-memory-entries?sessionId=${sessionId}`,
